...
async function main() {
  const data = JSON.parse(fs.readFileSync("import-data-12th-chemistry.json", "utf-8"));

  const chemistrySubjectId = data[0].dbSubjectId;
  console.log("Subject ID:", chemistrySubjectId);
  console.log("Deleting existing Class 12 Chemistry chapters...");

  const deleted = await prisma.chapter.deleteMany({
    where: { subjectId: chemistrySubjectId },
  });
  ...
  console.log(`\n=== Import Complete (Class 12 Chemistry) ===`);
